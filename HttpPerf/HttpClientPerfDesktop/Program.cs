...
namespace HttpClientPerf
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            // type [null|httpclient|httpwebrequest|winhttphandler]
            Func<int, PerfTest> factory = (index) =>
            {
                return new DesktopHttpClientPerfTestAsync(writer, index, wallTime, server, iterations, requestSizeCharacters);
            };
            ...
            if (args.Length > 6)
            {
                ...
                else if (type == "httpwebrequest")
                {
                    factory = (index) =>
                    {
                        return new DesktopHttpWebRequestPerfTestAsync(writer, index, wallTime, server, iterations, requestSizeCharacters);
                    };
                }
                else if (type == "winhttphandler")
                {
                    factory = (index) =>
                    {
                        return new WinHttpHandlerPerfTestAsync(writer, index, wallTime, server, iterations, requestSizeCharacters);
                    };
                }
            }
